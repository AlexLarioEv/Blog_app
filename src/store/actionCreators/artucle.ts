...
export const fetchArticle = (count: number, offset = 0, token?: string) => {
  return async (dispatch: Dispatch<TArticleAction>) => {
    const res = await fetch(`https://blog.kata.academy/api/articles?limit=${count}&offset=${offset}`, {
      method: 'GET',
      headers: {
...
export const fetchFavoritesArticle = (token: string, slug: string, favorited: boolean) => {
  return async (dispatch: Dispatch<TArticleAction>) => {
